...
bool decode_ringct(const rct::rctSig& rv, const crypto::public_key pub, const crypto::secret_key &sec, unsigned int i, rct::key & mask, uint64_t & amount, hw::device &hwdev)
{
    ...
    try
    {
        ...
        case rct::RCTTypeSimple:
            amount = rct::decodeRctSimple(rv, rct::sk2rct(scalar1), i, mask, hwdev);
            break;
        case rct::RCTTypeFull:
            amount = rct::decodeRct(rv, rct::sk2rct(scalar1), i, mask, hwdev);
            break;
        default:
...
bool lookup_account_outputs_ringct(const account_keys &acc, const transaction &tx, std::vector<std::pair<size_t, uint64_t> > &outputs, uint64_t &total_transfered, hw::device &hwdev)
{
  ...
  for(const tx_out& o:  tx.vout) {
      ...
      if (is_out_to_acc(acc, boost::get<txout_to_key>(o.target), tx_pub_key, additional_tx_public_keys, output_idx)) {
          ...
          if (decode_ringct(tx.rct_signatures, tx_pub_key, acc.m_view_secret_key, output_idx,
                            mask, rct_amount, hwdev)) {
             total_transfered += rct_amount;
             outputs.push_back(std::make_pair(output_idx, rct_amount));
          }
