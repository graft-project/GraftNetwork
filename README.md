...
build the library binary manually. This can be done with the following command ```sudo apt-get update && sudo apt-get install -y build-essential libssl-dev pkg-config libboost-all-dev``` then ```sudo apt-get install libgtest-dev && cd /usr/src/gtest && sudo cmake . && sudo make && sudo mv libg* /usr/lib/ ```
