...
bool is_valid_stake(uint64_t block_height, uint64_t stake_block_height, uint64_t stake_unlock_time)
{
  ...
  uint64_t stake_first_valid_block = stake_block_height + StakeTransactionProcessor::STAKE_VALIDATION_PERIOD,
           stake_last_valid_block  = stake_block_height + stake_unlock_time + StakeTransactionProcessor::TRUSTED_RESTAKING_PERIOD;
...
void BlockchainBasedList::apply_block(uint64_t block_height, const crypto::hash& block_hash, StakeTransactionStorage& stake_txs_storage)
{
  ...
  for (size_t i=0; i<StakeTransactionProcessor::TIERS_COUNT; i++)
  {
